...
@dataclass
class RegressionDataIngestionConfig:
    train_data_path: str=os.path.join('artifact','regression','train.xlsx')
    test_data_path: str=os.path.join('artifact','regression','test.xlsx')
    raw_data_path: str=os.path.join('artifact','regression','data.xlsx')

class RegressionDataIngestion:
    def __init__(self):
        
        self.ingenstion_config = RegressionDataIngestionConfig()

    

    def initiate_data_ingestion(self):
        logging.info(f'{"-"*10}Regression Data Ingestion initiated{"-"*10}')
        logging.info("Entered the data ingestion method or components")
        try:
            
            data = RawData('notebook/raw_data/data.xlsx')
            df = data.regression_data() 
